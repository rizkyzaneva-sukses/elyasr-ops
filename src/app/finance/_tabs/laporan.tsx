          ...
          {reportType==='pl' && data && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              ...
              <div className="space-y-4 max-w-3xl mx-auto text-sm">
                <div className="flex justify-between font-semibold text-emerald-400 text-base border-b border-zinc-800 pb-2"><span>Pendapatan Kotor (Omzet)</span><span>{formatRupiah(data.pendapatanKotor,true)}</span></div>
                <div className="flex justify-between pl-4 text-zinc-300"><span>Harga Pokok Penjualan (HPP)</span><span className="text-red-400">({formatRupiah(data.hpp,true)})</span></div>
                ...
                <div className="flex justify-between pt-2"><span className="font-semibold text-zinc-200">Biaya Penjualan (Fee Platform)</span><span className="text-red-400">({formatRupiah(data.feePlatform,true)})</span></div>
                <div className="flex justify-between font-semibold text-zinc-200 pb-2"><span>Beban Operasional</span><span className="text-red-400">({formatRupiah(data.bebanOperasional,true)})</span></div>
                {(data.expenseGroups||[]).map((g:any)=>(
                  <div key={g.group} className="flex justify-between pl-8 text-zinc-400 text-xs"><span>- {g.group}</span><span>{formatRupiah(g.amount,true)}</span></div>
                ))}
