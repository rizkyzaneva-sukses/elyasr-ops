    ...
    <AppLayout>
      <div className="page-header">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-0.5">{total.toLocaleString('id')} buyer unik</p>
        </div>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th className="w-28">Nama Penerima</th>
            ...
            <tbody>
              {isLoading ? Array.from({length:8}).map((_,i)=>(
                ...
                <tr><td colSpan={8} className="text-center py-12 text-zinc-600">
                  ...
                  <p>Tidak ada data buyer</p>
                </td></tr>
              ) : buyers.map((b: any, i: number) => (
                <tr key={b.buyer_key}>
                  <td className="text-zinc-600 text-xs">{(page-1)*limit + i + 1}</td>
                  <td>
                    <p className="text-sm text-zinc-200">{b.buyer_username || '—'}</p>
                  </td>
                  <td className="text-xs text-zinc-400">{b.receiver_name || '—'}</td>
                  <td>
                  ...
                  <td className="text-center">
                    ...
                    {b.totalOrders >= 3 && <span className="text-[10px] text-emerald-600 ml-1">★</span>}
                  </td>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-zinc-800">
            <p className="text-xs text-zinc-500">{total} buyer</p>
