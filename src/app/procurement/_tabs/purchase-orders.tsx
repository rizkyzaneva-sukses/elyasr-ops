...
function POFormModal({ vendors, editPO, onClose }: { vendors: any[]; editPO: any | null; onClose: () => void }) {
  ...
  const [items, setItems]             = useState<{sku:string;productName?:string;qtyOrder:number}[]>(
    isEdit ? (editPO.items?.map((it:any)=>({sku:it.sku,productName:it.productName,qtyOrder:it.qtyOrder})) || [{sku:'',qtyOrder:1}]) : [{sku:'',qtyOrder:1}]
  )
            ...
            <div className="space-y-2">
              {items.map((item, i) => (
                <div key={i} className="flex gap-2">
                  <POItemSelect item={item} onSelect={(sku) => updateItem(i, 'sku', sku)} />
